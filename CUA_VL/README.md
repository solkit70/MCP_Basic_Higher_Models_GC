...
#### 1. **vl_prompts/** 폴더
- **목적**: Topic-agnostic한 범용 프롬프트 템플릿 저장
- **핵심 파일**:
  - `roadmap_prompt.md`: 전체 학습 로드맵 생성용
  - `daily_learning_prompt.md`: 당일 학습 계획 수립용
  - `cvl_prompt.md` (선택): Continuous Vibe Learning용

**특징**:
- 특정 Topic에 종속되지 않음
- Topic만 전달하면 해당 주제의 계획 수립 가능
- 재사용 가능하여 다음 학습에도 동일하게 활용
- `vl_` 접두사로 CUA_VL 관련 폴더임을 명확히 표시
...
- 매 학습 세션마다 참조
- 학습 진행 중 업데이트 가능 (CVL)
- `vl_` 접두사로 방법론 전용 폴더임을 명시

#### 3. **vl_worklog/** 폴더
- **목적**: 일별 학습 작업 로그 및 진행 상황 기록
- **파일명 규칙**: `YYYYMMDD_[작업내용설명].md`
  - 예: `20251214_Day1_Setup.md`
  - 예: `20251215_M2_Environment.md`
